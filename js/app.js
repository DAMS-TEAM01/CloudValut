...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.addEventListener('contextmenu', (e) => {
        ...
        if (card && contextMenu) {
            ...
            contextMenu.dataset.targetName = card.querySelector('.file-name, span:not(.file-meta)') ? card.querySelector('.file-name, span:not(.file-meta)').innerText : 'Unknown';
    ...
    contextActions.forEach(action => {
        action.addEventListener('click', () => {
            ...
            if (actionType === 'star') {
                ...
                if (targetElement) {
                    if (isStarredPage) {
                        if (confirm(`Remove "${targetName}" from Starred?`)) {
                            removeFromStarred(targetName);
                            targetElement.remove();
                            showToast(`Removed "${targetName}" from Starred`);
                        }
                    } else {
                        // Standard delete
                        if (isTrashPage) {
                            ...
                            targetElement.remove();
                            removeFromStarred(targetName);
                            showToast(`Moved "${targetName}" to Trash`);
                        }
                    }
                }
            } else {
    ...
    if (emptyTrashBtn) {
        emptyTrashBtn.addEventListener('click', () => {
            const files = document.querySelectorAll('.grid-row .file-card, .grid-row .folder-card'); // Also include folders
            if (files.length > 0) {
